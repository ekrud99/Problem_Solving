...
int main(void){
    ...
    pq.push(pii(0, S)); // 시작 node push, pii.first가 0이므로 가장 높은 우선순위를 가짐
    dist[S] = 0; // 자신까지의 거리는 0
    ...
    while(!pq.empty()){
        ...
        rep(i, 0, graph[cur_node].size()){ // 선택된 node 기준 dist 갱신
            ...
            if(dist[next_node] > dist[cur_node] + weight){ // 기존 dist 보다 선택된 node 거쳐가는 경우가 이득인 경우 갱신
                ...
                route[next_node] = cur_node; // 갱신이 일어날 때 마다 어느 정점을 거쳤는지 기록
            }
        }
    }

    cout << dist[D] << endl; // 목적지 까지 가는 최소 비용

    int cur_node = D; // 목적지부터 역추적

    while(cur_node != S){ // 시작 node를 만날 때 까지 경로를 담아줌
        path.pb(cur_node);
        cur_node = route[cur_node];
    }

    path.pb(S); // 포함 안된 시작노드 push

    reverse(path.begin(), path.end()); // 역추적된 경로이므로 reverse

    cout << path.size() << endl; // 경로에 포함된 node의 개수 = path vectore의 size

    for(int node : path) cout << node << " "; // 경로 출력
